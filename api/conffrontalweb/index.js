...
/** Objeto de llamadas API para la sección 'Configuración front web'
...
export default ($axios, $config, store) => ({
  /** Obtener configuración front web
   * @param {Object} body Payload (Parámetros API)
   */
  async getConfiguracionFrontalWeb(body) {
    const params = getParamsForRequest($config, body, store);
    return await $axios.$post('/ConfiguracionFrontal/ObtenerConfiguracionFrontalWeb', params);
  },
});